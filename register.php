...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register') {
    ...
    if (empty($cookieHash) || !hash_equals($cookieHash, $inputHash)) {
        $register_error = "Incorrect Security Answer.";
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL) || !$password || !$name) {
        $register_error = "Please fill in all fields correctly.";
    } else {
        ...
        if ($stmt->fetch()) {
            ...
            if ($stmt->execute([$name, $email, $passHash])) {
                ...
                // COOKIE AUTH
                $authHash = hash_hmac('sha256', $uid, $secret);
                $cookieValue = $uid . ':' . $authHash;
...
<head>
    ...
    <title>Create Account | PriceScope</title>
...
<body class="h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8 relative">
            ...
            <h2 class="text-3xl font-bold">Join the Squad!</h2>
            <p class="text-slate-400">Start tracking prices like a pro.</p>
        </div>
        ...
        <form method="POST" class="space-y-4 neon-border p-8 rounded-3xl shadow-2xl">
            ...
            <div>
                <label class="block text-xs font-bold text-cyan-400 uppercase tracking-wider mb-2">Full Name</label>
                <input type="text" name="name" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-4 outline-none focus:border-cyan-400 transition-colors text-white placeholder-slate-600" placeholder="John Doe" required>
            </div>
        ...
        <p class="text-center mt-6 text-slate-500 text-sm">Already have an account? <a href="login.php" class="text-cyan-400 hover:text-cyan-300 font-bold">Login</a></p>
    </div>
